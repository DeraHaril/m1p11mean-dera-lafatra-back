...
module.exports = (rendez_vousCollection) => {
    insertRendez_vous = async(date, id_service, id_client, id_employe) => {
        try{
          const dateActuelle = new Date().toISOString();
            const insertRDV = await rendez_vousCollection.insertOne({ 
    ...
    getAllRendez_vous = async() => {
      try{
          const allRDV = await rendez_vousCollection.aggregate([
            {
              $lookup: {
                from: 'user',
    ...
    getAllRendez_vous = async(realized) => {
      try{
        const allRDV = await rendez_vousCollection.aggregate([
          {
            $match: {
    ...
    return{
      ...
      listeRendez_vousEffectue: async(req, res) => {
        try{
            const liste_rdv = await getAllRendez_vous(true);
      ...
      ajoutRendez_vous: async(req,res) =>{
        ...
        if(date <= dateActuelle.toISOString()){
          res.status(400).json({ success: false, message: "La date de rendez-vous ne doit pas précéder la date à laquelle vous avez ajouté le rendez-vous"});
        }
        try{
            var ajoutRDV = null;
            if(id_employe == null){
            ...
            if(ajoutRDV){
              res.status(201).json({ success: true, message: 'Rendez-vous ajouté avec succès', insertedId: ajoutRDV.insertedId });
            }else{
